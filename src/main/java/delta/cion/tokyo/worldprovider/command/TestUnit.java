...
public class TestUnit extends DeltaCommand {
	...
	private void spawnMob(CommandSender sender, CommandContext context) {
		...
		LOGGER.debug("Block spawned on {}", WorldProvider.getMobPosition());
		world.setBlock(WorldProvider.getMobPosition().withY(49), Block.BEDROCK);
		...
		TEST_UNIT.setInstance(world, WorldProvider.getMobPosition());
		LOGGER.debug("Unit instance is {}, with Pos on {}", world, WorldProvider.getMobPosition());
		TEST_UNIT.lookAt(WorldProvider.getSpawnPosition().withY(51));
		LOGGER.debug("Unit look at {}", WorldProvider.getSpawnPosition().withY(51));
	...
	private void killMob(CommandSender sender, CommandContext context) {
		...
		TEST_UNIT = null;
		Pos mobPosition = WorldProvider.getMobPosition().withY(49);
